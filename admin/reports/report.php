...
$todate=$data->todate;
$user_id=$data->recep_id;
if($user_id=="All")
{
$sql="select op.bill_no,CONCAT('OP ','Billing') as description, op.date,op.totalamt,op.paymentmode,op.cashamt as cashamt,op.cardamt as cardamt from op_make_payment op where op.date between '{$fromdate}' and '{$todate}' UNION select mp.bill_no, CONCAT('IP ','Billing') as description, mp.date ,mp.totalamt,mp.paymentmode,mp.cashamt as cashamt,mp.cardamt as cardamt from make_payment mp where mp.date between '{$fromdate}' and '{$todate}' UNION SELECT p.package_billno as bill_no,CONCAT('Packaage ','Billing') as description,p.date,p.totalamount as totalamt,p.paymentmode,p.cashamt as cashamt,p.cardamt as cardamt from op_lab_package_taken p where p.date between '{$fromdate}' and '{$todate}' UNION SELECT t.lab_billno as bill_no, CONCAT('Test ','Billing') as description,t.date,t.amount as totalamt,t.paymentmode,t.cashamt as cashamt,t.cardamt as cardamt from op_lab_tests_taken t where t.date between '{$fromdate}' and '{$todate}'";
